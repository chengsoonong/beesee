...
class Rectangle(object):
    def __init__(self, mat_array):
        self.x1, self.y1, self.x2, self.y2 = mat_array
        # print(mat_array)
        self.centre = (self.x1+self.x2)/2.0, (self.y1+self.y2)/2.0
...
class Detection(object):
    def __init__(self, mat_array):
        ...
        self.rectangles = []
        # for i in range(0, len(mat_array-2), 4):
        #     rect = mat_array[i:i+4]

        self.rectangles = [Rectangle(mat_array[i*4:i*4+4])
